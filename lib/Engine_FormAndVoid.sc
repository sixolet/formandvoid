	...
	alloc {
	    ...
	    [\form, \formAA].do{ |tag|
            SynthDef(tag, { |out, freq=440, amp=1, gate=1, pan=0,
	            ...
	            var f0 = freq*(1 + (e1F0.lag(0.1)*env1) + (e2F0.lag(0.1)*env2) + (lfoF2.lag(0.1)*lfo));
	            ...
	            var f1IndexMod = f1Index.lag(0.1) + (e1F1Index.lag(0.1)*env1) + (e2F1Index.lag(0.1)*env2) + (lfoF1Index.lag(0.1)*lfo);
	            ...
	            var f2IndexMod = f2Index.lag(0.1) + (e1F2Index.lag(0.1)*env1) + (e2F2Index.lag(0.1)*env2) + (lfoF2Index.lag(0.1)*lfo);
                ...
                if (tag == \formAA, {
	                ...
	                formant1 = f1AmpMod*Mix.new(FMGrainI.ar([imp, impDel], f1Res*f1Mod.reciprocal, f1Mod, f1Modulator, f1Index, silentBuf, winBuf, [1-phase, phase]));
	                formant2 = f2AmpMod*Mix.new(SinGrainI.ar([imp, impDel], f2Res*f2Mod.reciprocal, f2Mod, silentBuf, winBuf, [1-phase, phase]));
	            }, {
